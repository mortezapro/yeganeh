...
@section("content")
    <div class="content">
        <form action="{{ route("admin.products.store",["product"=>$product->slug]) }}" method="post" enctype="multipart/form-data">
            @csrf
                            ...
                            <div class="card-body">
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane active" id="main">
                                        <div class="row">
                                            ...
                                            <div class="col-md-4 mb-3">
                                                ...
                                                <select name="category[]" id="category" class="form-control select2" multiple>
                                                    @foreach($categories as $item)
                                                        <option @if($product->categories->contains($item)) selected @endif value="{{ $item->id }}">{{ $item->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-4 mb-3">
                                                ...
                                                @foreach($brands as $item)
                                                        <option @if($product->brand && $product->brand->id == $item->id) selected @endif value="{{ $item->id }}">{{ $item->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-4 mb-3">
                                            ...
                                            <div class="mb-3 col-md-12">
                                                ...
                                                <textarea type="text" name="overview" id="overview" class="form-control">{{ old("overview",$product->overview) }}</textarea>
                                            </div>
                                            <div class="col-md-12 mb-3">
                                                <label for="expert_review" class="form-label">نقد و بررسی فنی</label>
                                                <textarea name="expert_review" id="expert_review" class="form-control">{{ old("expert_review",$product->expert_review) }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="tab-pane" id="seo">
                                        <div class="row">
                                            <div class="mb-3 col-md-6">
                                                ...
                                                <input type="text" name="canonical" id="canonical" class="form-control" value="{{ old("canonical",$product->canonical) }}">
                                            </div>
                                            <div class="mb-3 col-md-6">
                                            ...
                                            <div class="mb-3 col-md-12">
                                                ...
                                                <textarea type="text" name="seo_description" id="seo_description" class="form-control">{{ old("seo_description",$product->seo_description) }}</textarea>
                                            </div>
                                    ...
                                    <div class="tab-pane" id="product-details">
                                        <div class="row">
                                            <table class="table table-striped table-responsive text-center position-relative align-middle" id="table-details">
                                                <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>ویژگی</th>
                                                ...
                                                @php
                                                  $valueService = \Illuminate\Support\Facades\App::make(\App\Services\ProductValue\ProductValueService::class);
                                                @endphp

                                                @foreach($product->details as $detail)
                                                    @php
                                                        $values = \App\Models\ProductValueModel::where("product_key_id","=",$detail->value->product_key_id)->get();
                                                    @endphp
                                                    <tr data-tr-row="{{ $loop->iteration }}">
                                                        ...
                                                        <td>
                                                            <select class="form-control product-key select2" data-id="1" style="width:100%">
                                                                ...
                                                                @foreach($keys as $key)
                                                                    <option @if($key->id == $detail->value->key->id) selected @endif value="{{ $key->id }}">{{ $key->name }}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <select name="product-value[]" class="form-control w-100 product-value{{ $loop->iteration }} select2">
                                                                <option value="-1">لطفا یک کلید انتخاب کنید</option>
                                                                @foreach($values as $value)
                                                                    <option @if($value->id == $detail->value->id) selected @endif value="{{ $value->id }}">{{ $value->value }}</option>
                                                                @endforeach
                                                            </select>
                                                        </td>
                                                        <td>
                                                            <input type="text" name="product-price[]" class="form-control product-price" value="{{ old("product-price",$detail->price) }}">
                                                        </td>
                                                        <td>
                                                            <input class="form-check-input rounded-circle" type="checkbox" name="selectable[{{ $loop->iteration-1 }}][]" id="selectable{{ $loop->iteration }}" @if($detail->selectable == "بله") checked @endif>
                                                            <label for="selectable{{ $loop->iteration }}" class="form-label">بله</label>
                                                        </td>
                                                        <td>
                                                            <a href="#"  class="btn btn-danger delete-row" data-row="{{ $loop->iteration }}">حذف سطر</a>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
...
@section("script")
    ...
    <script>
        ...
        @if(count($product->details) > 0)
            detailCount ={{ count($product->details) }};
        @endif
        $(document).on("click","#add-new-detail",(function( event ) {
            ...
            var newDetailData =
                ...
                '</td>' +
                '<td>' +
                    '<a href="#" class="btn btn-danger delete-row" data-row="'+detailCount+'">حذف سطر</a>' +
                '</td>' +
                '</tr>';
