...
<title>ویرایش پروژه | {{ env("APP_NAME") }}</title>
@section("head")
@endsection
@section("content")
            ...
            <div class="col-12">
                <div class="page-title-box">
                    ...
                    <div class="page-title-left">
                        <h4 class="page-title d-inline me-2">ویرایش پروژه</h4>
                        <a href="{{ route("panel.projects.index") }}" class="btn btn-primary"><i class="fe-list"></i> لیست پروژه‌ها</a>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <form action="{{ route("panel.projects.update",["project"=> $project->slug]) }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="container-fluid">
                    <div class="row">
                                        ...
                                        <div class="tab-pane active" id="main">
                                            <div class="row">
                                                ...
                                                <div class="col-md-12 mb-3">
                                                    ...
                                                    <textarea name="content" id="content" class="form-control">{{ old("content",$project->content) }}</textarea>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="tab-pane" id="seo">
                                            <div class="row">
                                                <div class="mb-3 col-md-6">
                                                    ...
                                                    <input type="text" name="canonical" id="canonical" class="form-control" value="{{ old("canonical",$project->canonical) }}">
                                                </div>
                                                <div class="mb-3 col-md-6">
                                                    <label for="seo_title" class="form-label">عنوان سئو</label>
                                                    <input type="text" name="seo_title" id="seo_title" class="form-control" value="{{ old("seo_title",$project->seo_title) }}">
                                                </div>
                                                <div class="mb-3 col-md-12">
                                                    <label for="seo_description" class="form-label">توضیحات سئو</label>
                                                    <textarea type="text" name="seo_description" id="seo_description" class="form-control">{{ old("seo_description",$project->seo_description) }}</textarea>
                                                </div>
                                                <div class="gallery-preview d-flex justify-content-center" id="seo-image-preview">
                                                    <img class="gallery-preview-item" src="{{ asset("seo-images/default")."/".$project->seo_image }}">
                                                </div>
                                                <div class="mb-3">
                                                ...
                                                <div class="mb-3">
                                                    <input class="form-check-input rounded-circle" type="checkbox" name="indexable" id="indexable" @if($project->indexable) checked @endif value="1" >
                            ...
                            <div class="card sticky-top">
                                <div class="card-body">
                                    <div class="gallery-preview d-flex justify-content-center" id="thumbnail-preview">
                                        <img class="gallery-preview-item" src="{{ asset("project-thumbnails/default")."/".$project->thumbnail }}">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <div class="col-md-12 mb-3">
                                        <label for="project_type" class="form-label">نوع پروژه</label>
                                        <select name="type" id="project_type" class="form-control">
                                            <option @if($project->type == config("project-type.scan")) selected @endif value="{{ config("project-type.scan") }}">اسکن</option>
                                            <option @if($project->type == config("project-type.software")) selected @endif value="{{ config("project-type.software") }}">نرم‌افزار</option>
                                        </select>
                                    </div>
                                    <div class="mb-3">
                                        <label for="status" class="form-label d-block mb-3">وضعیت</label>
                                        <div class="btn-group">
                                            <input value="{{ config("project-status.done") }}" type="radio" class="btn-check" name="status" id="status1" @if($project->status == config("project-status.done")) checked @endif>
                                            <label class="btn btn-outline-primary" for="status1">پایان‌یافته</label>
                                            <input value="{{ config("project-status.in-progress") }}" type="radio" class="btn-check" name="status" id="status2" @if($project->status == config("project-status.in-progress")) checked @endif>
                                            <label class="btn btn-outline-primary" for="status2">در حال انجام</label>
                                        </div>
                                    </div>
                                    <div class="mb-3">
